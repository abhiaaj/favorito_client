...
class BusinessProfileProvider extends BaseProvider {
  ...
  String getShopTime() {
    ...
    print("time:${_businessProfileData?.data[0].businessStatus}");
    return _shopTiming == '' ||
            _businessProfileData?.data[0].businessStatus == 'offline'
        ? _shopTiming = "Closed Now" + '\u{25BC}'
        : _shopTiming;
  }
          ...
          try {
            ...
            DateTime startTime = properSlot == updated
                ...
                : properSlot.difference(updated).isNegative
                    ? updated
                    : properSlot;
            print("sss4 : ${value.data[0].wait_time}");
            remainTime = value.data[0].wait_time.toString().split('.')[0];
            //print(remainTime);
  ...
  void getHighlightsPhotos() async {
    Map _map = {'business_id': this.getBusinessId()};
    print("business_id_new: ${this.getBusinessId()}");
    gethighlightsdata.clear();
    await APIManager.getHighlights(_map).then((value) {
      if (value.status == 'success') {
        gethighlightsdata.addAll(value.data.photos);
        print("length: ${gethighlightsdata.length}");
        notifyListeners();
      }
    });
  }
